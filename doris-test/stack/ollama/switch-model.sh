...
TARGET="${1:-}"
COMPOSE_DIR="${COMPOSE_DIR:-/home/ec2-user/doris-test/compose}"
...
SQL_MODEL="${OLLAMA_SQL_MODEL:-a-kore/Arctic-Text2SQL-R1-7B}"
...
if ! curl -sf "${OLLAMA_URL}/api/tags" | grep -Fq "$TARGET"; then
  ...
  if ! wait "$pull_pid"; then
    echo "PULL_FAIL: ollama pull $TARGET" >&2
