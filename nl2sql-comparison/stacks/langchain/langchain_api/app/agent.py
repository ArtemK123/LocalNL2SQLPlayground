...
def _arctic_dialect() -> str:
    d = (settings.arctic_sql_dialect or "sqlite").strip().lower()
    return "postgresql" if d in {"postgresql", "postgres", "pg"} else "sqlite"


def _arctic_engine_label() -> str:
    return "PostgreSQL" if _arctic_dialect() == "postgresql" else "SQLite"


# Prefill used by official Arctic eval / Study-path serving: skip long CoT by
# continuing past <think> into the opening ```sql fence so a 10s wall-clock
# still yields executable SQL.
def _arctic_assistant_prefill() -> str:
    engine = _arctic_engine_label()
    return (
        "Let me solve this step by step.\n"
        "<think>\n"
        f"I will write a concise {engine} query that answers the question.\n"
        "</think>\n"
        "<answer>\n"
        "```sql\n"
    )
...
# Back-compat name used by tests / logs.
ARCTIC_ASSISTANT_PREFILL = _arctic_assistant_prefill()

# Stop as soon as the SQL fence / answer closes (generate/correct only).
...
def _build_chat_llm(model: str) -> BaseChatModel:
    ...
    if backend == "vllm":
        ...
        if not base.endswith("/v1"):
            base = f"{base}/v1"
        # LangChain docs: base_url must include /v1; api_key required even if unused.
        # Timeout must stay finite; harness timeout_sec usually cuts first.
        return ChatOpenAI(
            model=model,
            base_url=base,
...
SQL_PROMPT = ChatPromptTemplate.from_messages(
    [
        (
            "system",
            (
                "You are a PostgreSQL NL2SQL assistant for the BIRD benchmark.\n"
                "Generate exactly one read-only SELECT query.\n"
                "Rules:\n"
                "- Use only these PostgreSQL schemas: {allowed_schemas}\n"
...
# Arctic-Text2SQL-R1: OmniSQL-style prompt (arxiv 2505.20315 / ArcticTraining bird_eval).
# Dialect/engine label comes from ARCTIC_SQL_DIALECT (Study default: SQLite).
# vLLM path appends assistant SQL-fence prefill + continue_final_message; Ollama cannot
# prefill reliably so it keeps an explicit "start your reply" instruction instead.
...
ARCTIC_REPAIR_PROMPT = ChatPromptTemplate.from_messages(
    [
        (
            "system",
            (
...
REPAIR_PROMPT = ChatPromptTemplate.from_messages(
    [
        (
            "system",
            (
                "You are fixing a PostgreSQL SELECT query for execution correctness.\n"
...
def _is_arctic_model(model: str) -> bool:
    lowered = model.lower()
    # Matches Hub a-kore/Arctic-Text2SQL-… and local arctic-text2sql-r1-7b:q4_k_m
...
class SqlAgent:
    def __init__(self, engine: Engine) -> None:
        ...
        # Do not stop on </sql>: official Arctic outputs ```sql fences (infer.py parse_response).
    ...
    def _bind_arctic_sql_llm(self, llm: BaseChatModel) -> BaseChatModel:
        """Attach vLLM continue_final_message + stop so generation ends after SQL."""
        ...
        prompt = ChatPromptTemplate.from_messages(
            [
                (
                    "system",
                    (
                        "Select the most relevant tables for an NL2SQL task.\n"
    ...
    def _generate_sql_with(
        ...
        if isinstance(raw, list):
            raw = "".join(str(part) for part in raw)
        raw_text = str(raw)
        # Full db_id-scoped catalog: only block DDL/non-SQL; allow any table in that database.
        table_allowlist = (
            None
            if selection.used_fallback_full_schema
            else selection.selected_table_fq
        )
        try:
            return validate_sql(
    ...
    def generate_sql(
        ...
        *,
        ...
        # Single LLM call per question (no fallback re-generation).
        return (
    ...
    def repair_sql(
        ...
        *,
        ...
        repaired = validate_sql(
            extract_sql(str(raw)),
            allowed_tables=selection.allowed_table_fq,
            allowed_schemas=self.allowed_schemas,
            dialect=self.sql_dialect,
        )
